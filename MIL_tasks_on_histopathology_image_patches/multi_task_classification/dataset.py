...
class Dataset(torch.utils.data.Dataset):
	def __init__(self, dataset_dir=None, dataset_type='train', patch_size = 32, num_instances=10):
		...
		unique_ids, unique_id_counts = np.unique(self._labels_arr, return_counts=True, axis=0)
		...
		for i in range(unique_ids.shape[0]):
			...
			temp_bool = (self._labels_arr == temp_unique_id)
			temp_bool_sum = np.sum(temp_bool,axis=1)

			temp_indices = np.where(temp_bool_sum == 2)[0]
	...
	def label_fnc(self, metastasis_ratio_arr, ucc_arr):
		# 10:normal, 01:metastases, 11:boundary
		labels_arr = np.zeros((ucc_arr.shape[0],2), dtype=np.uint8)
		
		# find normal images
		ucc1_fully_normal_indices = (ucc_arr == 1) & (metastasis_ratio_arr<0.5)
		labels_arr[ucc1_fully_normal_indices] = [1,0]

		# find metastases images
		ucc1_fully_metastases_indices = (ucc_arr == 1) & (metastasis_ratio_arr>0.5)
		labels_arr[ucc1_fully_metastases_indices] = [0,1]

		# find boundary images
		ucc2_indices = (ucc_arr == 2)
		labels_arr[ucc2_indices] = [1,1]
