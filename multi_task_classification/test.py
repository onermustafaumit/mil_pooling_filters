...
data_loader = torch.utils.data.DataLoader(dataset, batch_size=FLAGS.batch_size, shuffle=False, num_workers=10, collate_fn=custom_collate_fn)
...
with torch.no_grad():
	...
	for i in range(num_images):
		...
		with open(test_metrics_filename,'w') as f_metric_file:
			...
			f_metric_file.write('truth_normal\ttruth_metastases\t')
			f_metric_file.write('predicted_normal\tpredicted_metastases\t')
			f_metric_file.write('prob_normal\tprob_metastases\n')
		...
		for images, targets in data_loader:
			...
			batch_predicteds = np.asarray(batch_probs_arr > 0.5, dtype=int)
			...
			with open(test_metrics_filename,'a') as f_metric_file:
				for b in range(num_samples):
					...
					for c in range(FLAGS.num_classes):
						f_metric_file.write('\t{:d}'.format(batch_truths[b,c]))
					for c in range(FLAGS.num_classes):
						f_metric_file.write('\t{:d}'.format(batch_predicteds[b,c]))
					for c in range(FLAGS.num_classes):
