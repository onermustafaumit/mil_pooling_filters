...
train_data_loader = torch.utils.data.DataLoader(train_dataset, batch_size=FLAGS.batch_size, shuffle=True, num_workers=10, collate_fn=custom_collate_fn)
val_data_loader = torch.utils.data.DataLoader(val_dataset, batch_size=FLAGS.batch_size, shuffle=False, num_workers=10, collate_fn=custom_collate_fn)
...
with open(metrics_file,'w') as f_metric_file:
	...
	f_metric_file.write('# epoch\ttraining_acc\ttraining_loss\tvalidation_acc\tvalidation_loss\ttr_acc_normal\ttr_acc_metastases\tval_acc_normal\tval_acc_metastases\n')
...
for epoch in range(FLAGS.num_epochs):
	...
	num_corrects_arr = np.zeros(FLAGS.num_classes)
	num_predictions = 0
	...
	for images, targets in train_data_loader:
		...
		predicted_labels = y_logits > 0.0

		correct_predictions = torch.sum(predicted_labels == target_labels, dim=0)

		num_corrects_arr += correct_predictions.cpu().numpy()
	...
	training_acc_task_based = num_corrects_arr / num_predictions
	training_acc = np.mean(training_acc_task_based)
	...
	validation_acc_task_based = num_corrects_arr / num_predictions
	validation_acc = np.mean(validation_acc_task_based)

	pbar.close()

	print('Epoch=%d ### training_acc=%5.3f, training_loss=%5.3f ### validation_acc=%5.3f, validation_loss=%5.3f' % (epoch+1, training_acc, training_loss, validation_acc, validation_loss))

	with open(metrics_file,'a') as f_metric_file:
		f_metric_file.write('%d\t%5.3f\t%5.3f\t%5.3f\t%5.3f' % (epoch+1, training_acc, training_loss, validation_acc, validation_loss))
		for i in range(FLAGS.num_classes):
			f_metric_file.write('\t{:.3f}'.format(training_acc_task_based[i]))
		for i in range(FLAGS.num_classes):
			f_metric_file.write('\t{:.3f}'.format(validation_acc_task_based[i]))
		f_metric_file.write('\n')
