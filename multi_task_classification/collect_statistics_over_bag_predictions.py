...
with open(image_predictions_file_mpp,'w') as f_image_predictions_file_mpp:
	...
	f_image_predictions_file_mpp.write('truth_normal\ttruth_metastases\t')
	f_image_predictions_file_mpp.write('predicted_normal\tpredicted_metastases\t')
	f_image_predictions_file_mpp.write('prob_normal\tprob_metastases\n')
...
for i in range(num_images):
	...
	truths_data = np.asarray(test_metrics_data[:,1:3],dtype=int)
	preds_data = np.asarray(test_metrics_data[:,3:5],dtype=int)
	probs_data = np.asarray(test_metrics_data[:,5:],dtype=float)
	...
	image_pred = np.asarray(image_probs_mpp > 0.5, dtype=int)
	...
	with open(image_predictions_file_mpp,'a') as f_image_predictions_file_mpp:
		...
		f_image_predictions_file_mpp.write('{:d}\t'.format(image_truth[0]))
		f_image_predictions_file_mpp.write('{:d}\t'.format(image_truth[1]))
		f_image_predictions_file_mpp.write('{:d}\t'.format(image_pred[0]))
		f_image_predictions_file_mpp.write('{:d}\t'.format(image_pred[1]))
		f_image_predictions_file_mpp.write('{:.3f}\t'.format(image_probs_mpp[0]))
		f_image_predictions_file_mpp.write('{:.3f}\n'.format(image_probs_mpp[1]))
...
# normal
temp_truth = bag_truths_arr[:,0]
temp_pred = bag_preds_arr[:,0]

conf_mat = confusion_matrix(temp_truth, temp_pred, labels=[0,1])
conf_mat_filename = '{}/bag_level_cm_normal.txt'.format(data_folder_path)
np.savetxt(conf_mat_filename, conf_mat, fmt='%d', delimiter='\t')

acc_normal = np.sum(conf_mat.diagonal())/np.sum(conf_mat)

class_names = ['-ve','+ve']

fig, ax = plt.subplots(figsize=(2,2))
plot_confusion_matrix(conf_mat, classes=class_names, normalize=True, title='Confusion matrix', current_ax=ax)
fig_filename = '{}/bag_level_cm_normalized_normal.png'.format(data_folder_path)
fig.savefig(fig_filename, bbox_inches='tight')
plt.close('all')

fig, ax = plt.subplots(figsize=(2,2))
plot_confusion_matrix(conf_mat, classes=class_names, normalize=False, title='Confusion matrix', current_ax=ax)
fig_filename = '{}/bag_level_cm_unnormalized_normal.png'.format(data_folder_path)
fig.savefig(fig_filename, bbox_inches='tight')
plt.close('all')

bag_level_statistics_filename = '{}/bag_level_statistics_normal.txt'.format(data_folder_path)
...
# metastases
temp_truth = bag_truths_arr[:,1]
temp_pred = bag_preds_arr[:,1]

conf_mat = confusion_matrix(temp_truth, temp_pred, labels=[0,1])
conf_mat_filename = '{}/bag_level_cm_metastases.txt'.format(data_folder_path)
np.savetxt(conf_mat_filename, conf_mat, fmt='%d', delimiter='\t')

acc_metastases = np.sum(conf_mat.diagonal())/np.sum(conf_mat)

fig, ax = plt.subplots(figsize=(2,2))
plot_confusion_matrix(conf_mat, classes=class_names, normalize=True, title='Confusion matrix', current_ax=ax)
fig_filename = '{}/bag_level_cm_normalized_metastases.png'.format(data_folder_path)
fig.savefig(fig_filename, bbox_inches='tight')
plt.close('all')

fig, ax = plt.subplots(figsize=(2,2))
plot_confusion_matrix(conf_mat, classes=class_names, normalize=False, title='Confusion matrix', current_ax=ax)
fig_filename = '{}/bag_level_cm_unnormalized_metastases.png'.format(data_folder_path)
fig.savefig(fig_filename, bbox_inches='tight')
plt.close('all')

bag_level_statistics_filename = '{}/bag_level_statistics_metastases.txt'.format(data_folder_path)
with open(bag_level_statistics_filename, 'w') as f_bag_level_statistics_filename:
	f_bag_level_statistics_filename.write('# acc\n')
	f_bag_level_statistics_filename.write('{}\n'.format(acc_metastases))


# collect image level statistics - mpp
# normal
temp_truth = image_truths_arr[:,0]
temp_pred = image_preds_arr[:,0]

conf_mat = confusion_matrix(temp_truth, temp_pred, labels=[0,1])
conf_mat_filename = '{}/image_level_cm_mpp_normal.txt'.format(data_folder_path)
np.savetxt(conf_mat_filename, conf_mat, fmt='%d', delimiter='\t')

fig, ax = plt.subplots(figsize=(2,2))
plot_confusion_matrix(conf_mat, classes=class_names, normalize=True, title='Confusion matrix', current_ax=ax)
fig_filename = '{}/image_level_cm_normalized_mpp_normal.png'.format(data_folder_path)
fig.savefig(fig_filename, bbox_inches='tight')
plt.close('all')

fig, ax = plt.subplots(figsize=(2,2))
plot_confusion_matrix(conf_mat, classes=class_names, normalize=False, title='Confusion matrix', current_ax=ax)
fig_filename = '{}/image_level_cm_unnormalized_mpp_normal.png'.format(data_folder_path)
fig.savefig(fig_filename, bbox_inches='tight')
plt.close('all')

acc_normal = np.sum(conf_mat.diagonal())/np.sum(conf_mat)

image_level_statistics_filename = '{}/image_level_statistics_mpp_normal.txt'.format(data_folder_path)
...
# metastases
temp_truth = image_truths_arr[:,1]
temp_pred = image_preds_arr[:,1]

conf_mat = confusion_matrix(temp_truth, temp_pred, labels=[0,1])
conf_mat_filename = '{}/image_level_cm_mpp_metastases.txt'.format(data_folder_path)
np.savetxt(conf_mat_filename, conf_mat, fmt='%d', delimiter='\t')

fig, ax = plt.subplots(figsize=(2,2))
plot_confusion_matrix(conf_mat, classes=class_names, normalize=True, title='Confusion matrix', current_ax=ax)
fig_filename = '{}/image_level_cm_normalized_mpp_metastases.png'.format(data_folder_path)
fig.savefig(fig_filename, bbox_inches='tight')
plt.close('all')

fig, ax = plt.subplots(figsize=(2,2))
plot_confusion_matrix(conf_mat, classes=class_names, normalize=False, title='Confusion matrix', current_ax=ax)
fig_filename = '{}/image_level_cm_unnormalized_mpp_metastases.png'.format(data_folder_path)
fig.savefig(fig_filename, bbox_inches='tight')
plt.close('all')

acc_metastases = np.sum(conf_mat.diagonal())/np.sum(conf_mat)

image_level_statistics_filename = '{}/image_level_statistics_mpp_metastases.txt'.format(data_folder_path)
with open(image_level_statistics_filename, 'w') as f_image_level_statistics_filename:
	f_image_level_statistics_filename.write('# acc\n')
	f_image_level_statistics_filename.write('{:.4f}\n'.format(acc_metastases))
